...
workflow {
    ...
    knownsites = params.knownsites ? Channel.fromPath(params.knownsites) as List).collect() : Channel.value([])
    knownsites_indexes = params.knownsites_indexes ? Channel.fromPath(params.knownsites_indexes) as List).collect() : Channel.value([])
